    ...
    public String execute(List<String> args, CommandContext context)
                                ...
                                try {
                                    context.output().println(
                                            GameEndReporter.fetchAndFormat(
                                                    ...
                                                    context.session().accountToken(),
                                                    notification.gameId()));
                                } catch (IOException e) {
                                    context.output().println("Could not fetch game end summary: " + e.getMessage());
                                }
        ...
        ApiResponse<LoginData> response =
                (ApiResponse<LoginData>)
                        context
                                .connectionManager()
                                .send(new LoginRequest(args.get(1), args.get(2), context.config().udpPort()));
        if (!response.success()) {
        ...
        if (gameResponse.success()) {
            gameInfoMessage =
                    OutputFormatter.formatGameInfo(gameResponse.data(), System.currentTimeMillis());
        } else {
            gameInfoMessage =
                    "Warning: could not fetch current game info: "
                            + OutputFormatter.formatError(gameResponse.error().message());
        }
        return "Logged in as "
                + args.get(1)
                + ". Current game participation started.\n"
                + gameInfoMessage;
    }
}