...
public final class RequestLeaderboardCommand implements Command {
    ...
    public String execute(List<String> args, CommandContext context) throws CommandException, IOException {
        ...
        throw new CommandException("Usage: leaderboard | leaderboard top <K> | leaderboard player <name>");
    }

    private String requestLeaderboard(CommandContext context, String playerName, Integer topK)
            throws CommandException, IOException {
        @SuppressWarnings("unchecked")
        ApiResponse<LeaderboardData> response = (ApiResponse<LeaderboardData>) context.connectionManager().send(
            new RequestLeaderboardRequest(context.session().accountToken(), playerName, topK)
        );
        if (!response.success()) {
