...
/**
 * Displays the menu for logged-in users, providing options to play, view stats, update credentials,
 * or logout.
 */
public final class LoggedInMenu {
  ...
  /**
   * Constructs a new logged-in menu.
   *
  ...
  /**
   * Runs the menu loop until the user logs out or the session ends.
   *
  ...
  public void run() throws IOException {
    while (session.isLoggedIn()) {
      ...
      if (currentGameResponse.success()) {
        output.println();
        output.println(
            OutputFormatter.formatGameInfo(currentGameResponse.data(), System.currentTimeMillis()));
      } else {
        output.println();
        output.println("Current game info unavailable: " + currentGameResponse.error().message());
      }
  ...
  /**
   * Shows game information for a specified or current game.
   *
   * @throws IOException if an I/O error occurs
   */
  ...
  /**
   * Shows game statistics for a specified or current game.
   *
   * @throws IOException if an I/O error occurs
   */
  ...
  /**
   * Displays the leaderboard, optionally for a specific player or top N.
   *
   * @throws IOException if an I/O error occurs
   */
  private void leaderboard() throws IOException {
    ...
    ApiResponse<LeaderboardData> response =
        ClientActions.fetchLeaderboard(connectionManager, session.accountToken(), playerName, topK);
    if (response.success()) {
  ...
  /**
   * Shows statistics for the current player.
   *
   * @throws IOException if an I/O error occurs
   */
  ...
  /**
   * Prompts for new credentials and attempts to update them.
   *
   * @throws IOException if an I/O error occurs
   */
