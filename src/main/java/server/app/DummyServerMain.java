...
public final class DummyServerMain {
  ...
  private static final List<List<String>> WORD_GROUPS =
      List.of(
  ...
  private static final List<String> ALL_WORDS =
      WORD_GROUPS.stream().flatMap(Collection::stream).collect(Collectors.toUnmodifiableList());
        ...
        BufferedWriter writer =
            new BufferedWriter(
                new OutputStreamWriter(
                    client.socket().getOutputStream(), StandardCharsets.UTF_8))) {
        ...
        try {
          ...
          writer.write(
              GSON.toJson(
                  new ApiResponse<>(
                      false,
                      new ApiError(ErrorCode.INTERNAL_ERROR, "Server error: " + e.getMessage()),
                      null)));
          writer.newLine();
          writer.flush();
        }
  ...
  private ApiResponse<UpdateCredentialsData> update(UpdateCredentialsRequest request) {
    ...
    if (!request.oldUsername().equals(request.newUsername())
        && passwords.containsKey(request.newUsername())) {
  ...
  private ApiResponse<GameInfoData> submit(SubmitProposalRequest request) {
    ...
    Set<String> alreadyGrouped =
        playerGuesses.stream().flatMap(g -> g.words().stream()).collect(Collectors.toSet());
    ...
    boolean correct =
        WORD_GROUPS.stream().map(HashSet::new).anyMatch(g -> g.equals(new HashSet<>(words)));
  ...
  private ApiResponse<PlayerStatsData> playerStats(RequestPlayerStatsRequest request) {
    ...
    return ok(
        new PlayerStatsData(
            completed, winRate, lossRate, currentStreak, maxStreak, perfect, histogram));
  }
  ...
  private GameInfoData buildGameInfo(String username, long gameId, boolean expired) {
    ...
    return new GameInfoData(
        gameId, expiresAt(gameId), shuffledWords, correct, wrong, correctGroups);
  }
  ...
  private List<PlayerGuess> getOrCreateGuesses(String username) {
    return guesses.computeIfAbsent(
        key(username, currentGameId()), k -> new CopyOnWriteArrayList<>());
  }
    ...
    for (Map.Entry<String, Integer> entry : udpPorts.entrySet()) {
      ...
      try (DatagramSocket socket = new DatagramSocket()) {
        DatagramPacket packet =
            new DatagramPacket(payload, payload.length, InetAddress.getLoopbackAddress(), port);
        socket.send(packet);
      } catch (IOException e) {
