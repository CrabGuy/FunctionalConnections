...
public class NetworkingTest {
  ...
  public static void main(String[] args) {
    ...
    runTest(
        "testRegisterUsernameAlreadyRegistered",
        NetworkingTest::testRegisterUsernameAlreadyRegistered);
    ...
    runTest(
        "testUpdateCredentialsIncorrectPassword",
        NetworkingTest::testUpdateCredentialsIncorrectPassword);
    runTest(
        "testUpdateCredentialsNewUsernameTaken",
        NetworkingTest::testUpdateCredentialsNewUsernameTaken);
    ...
    runTest(
        "testSubmitProposalPlayerAlreadyCompleted",
        NetworkingTest::testSubmitProposalPlayerAlreadyCompleted);
    ...
    runTest(
        "testRequestLeaderboardInvalidToken", NetworkingTest::testRequestLeaderboardInvalidToken);
  ...
  private static void testRegisterUsernameAlreadyRegistered() {
    ...
    check(
        response.error().code() == ErrorCode.USERNAME_ALREADY_REGISTERED,
        "error code should be USERNAME_ALREADY_REGISTERED");
  ...
  private static void testLoginIncorrectPassword() {
    ...
    check(response.error() != null, "error should be present");
    check(
        response.error().code() == ErrorCode.INCORRECT_PASSWORD,
        "error code should be INCORRECT_PASSWORD");
  }
  ...
  private static void testUpdateCredentialsNewUsernameTaken() {
    ...
    check(!response.success(), "updateCredentials should fail");
    check(
        response.error().code() == ErrorCode.NEW_USERNAME_ALREADY_TAKEN,
        "error code should be NEW_USERNAME_ALREADY_TAKEN");
  }
  ...
  private static void testSubmitProposalSuccess() {
    ...
    ApiRequest request =
        new SubmitProposalRequest("token", List.of("red", "blue", "green", "yellow"));
    ApiResponse<?> response = dispatcher.dispatch(request, TEST_ADDRESS);
  ...
  private static void testSubmitProposalInvalidProposal() {
    ...
    check(
        response.error().code() == ErrorCode.MALFORMED_PROPOSAL,
        "error code should be MALFORMED_PROPOSAL");
  }
  ...
  private static void testSubmitProposalGameNotCurrent() {
    ...
    check(!response.success(), "submitProposal should fail");
    check(
        response.error().code() == ErrorCode.GAME_NOT_CURRENT,
        "error code should be GAME_NOT_CURRENT");
  }
  ...
  private static void testSubmitProposalPlayerAlreadyCompleted() {
    ...
    check(!response.success(), "submitProposal should fail");
    check(
        response.error().code() == ErrorCode.PLAYER_ALREADY_COMPLETED_GAME,
        "error code should be PLAYER_ALREADY_COMPLETED_GAME");
  }
  ...
  private static void testSubmitProposalInvalidToken() {
    ...
    ApiRequest request =
        new SubmitProposalRequest("bad-token", List.of("red", "blue", "green", "yellow"));
    ApiResponse<?> response = dispatcher.dispatch(request, TEST_ADDRESS);
    ...
    ProposalService proposalService =
        new NetworkingTestFactory.StubProposalService() {
          ...
          public GameInfoData getGameInfo(String accountToken, Long gameId) {
            return new GameInfoData(
                1L, 123456L, List.of("cat", "dog", "bird", "fish"), List.of(), List.of(), null);
          }
        };
    RequestDispatcher dispatcher =
        NetworkingTestFactory.createRequestDispatcher(
  ...
  private static void testRequestGameInfoGameNotFound() {
    ...
    check(
        response.error().code() == ErrorCode.GAME_NOT_FOUND, "error code should be GAME_NOT_FOUND");
  }
    ...
    LeaderboardService leaderboardService =
        new NetworkingTestFactory.StubLeaderboardService() {
          ...
          public LeaderboardData getLeaderboard(
              String accountToken, String playerName, Integer topK) {
