...
public class StatsTest {
  ...
  public static void main(String[] args) {
    ...
    runTest(
        "testPlayerStatsHistogramExcludesIncomplete",
        StatsTest::testPlayerStatsHistogramExcludesIncomplete);
    ...
    runTest(
        "testGameStatsActiveVsCompletedOngoing", StatsTest::testGameStatsActiveVsCompletedOngoing);
  ...
  private static void testGameStatsCompleted() {
    ...
    playerRepo.save(
        new PlayerGame("alice", gameId, List.of(CORRECT_A, CORRECT_B, CORRECT_C))); // won
    playerRepo.save(
        new PlayerGame("bob", gameId, List.of(CORRECT_A, WRONG_1, CORRECT_B, CORRECT_C))); // won
    playerRepo.save(
        new PlayerGame(
            "carol",
            gameId,
            List.of(
                CORRECT_A, CORRECT_B, WRONG_1, WRONG_1, WRONG_1, WRONG_1))); // lost (4 mistakes)
    playerRepo.save(
        new PlayerGame("dave", gameId, List.of(CORRECT_A, WRONG_1, WRONG_1))); // incomplete
  ...
  private static void testGameStatsOngoing() {
    ...
    playerRepo.save(
        new PlayerGame("bob", gameId, List.of(CORRECT_A, WRONG_1, CORRECT_B, CORRECT_C)));
    playerRepo.save(
        new PlayerGame(
            "carol", gameId, List.of(CORRECT_A, CORRECT_B, WRONG_1, WRONG_1, WRONG_1, WRONG_1)));
  ...
  private static void testPlayerStats() {
    ...
    playerRepo.save(
        new PlayerGame(
            "alice", 1L, List.of(CORRECT_A, WRONG_1, CORRECT_B, WRONG_1, CORRECT_C))); // win
    playerRepo.save(
        new PlayerGame("alice", 2L, List.of(WRONG_1, WRONG_1, WRONG_1, WRONG_1))); // loss
    playerRepo.save(
        new PlayerGame("alice", 3L, List.of(CORRECT_A, CORRECT_B, WRONG_1))); // incomplete
  ...
  private static void testLeaderboard() {
    ...
    check(
        top.get(0).username().equals("alice") && top.get(0).score() == 24,
        "First should be alice with 24");
    check(
        top.get(1).username().equals("dave") && top.get(1).score() == 18,
        "Second should be dave with 18");
    check(
        top.get(2).username().equals("bob") && top.get(2).score() == 16,
        "Third should be bob with 16");
    check(
        top.get(3).username().equals("carol") && top.get(3).score() == 2,
        "Fourth should be carol with 2");
    ...
    check(
        top2.topPlayers().get(0).username().equals("alice")
            && top2.topPlayers().get(0).score() == 24,
        "First of top2 should be alice");
    check(
        top2.topPlayers().get(1).username().equals("dave")
            && top2.topPlayers().get(1).score() == 18,
        "Second of top2 should be dave");
    ...
    check(
        carolData.requestedPlayer().username().equals("carol"),
        "requestedPlayer username should be carol");
  ...
  private static void testInvalidToken() {
    ...
    PlayerGameRepository playerRepo = StatsTestFactory.createPlayerGameRepository();
    AccountService accountService =
        StatsTestFactory.createAccountService(Map.of("valid-token", "alice"));
    StatsService statsService =
        StatsTestFactory.createStatsService(accountService, playerRepo, gameRepo, clock);
  ...
  private static void testPlayerStatsHistogramExcludesIncomplete() {
    ...
    playerRepo.save(
        new PlayerGame("alice", 0L, List.of(CORRECT_A, CORRECT_B, CORRECT_C))); // win, 0 wrong
    playerRepo.save(
        new PlayerGame("alice", 1L, List.of(CORRECT_A, CORRECT_B, WRONG_1))); // incomplete
    playerRepo.save(
        new PlayerGame(
            "alice",
            2L,
            List.of(CORRECT_A, WRONG_1, CORRECT_B, WRONG_1, CORRECT_C))); // win, 2 wrong
  ...
  private static void testLeaderboardTopKBounds() {
    ...
    check(
        top999.topPlayers().size() == 3, "topK larger than player count should return all players");
  }
  ...
  private static void testLeaderboardNonExistentPlayer() {
    ...
    PlayerGameRepository playerRepo = StatsTestFactory.createPlayerGameRepository();
    AccountService accountService =
        StatsTestFactory.createAccountService(Map.of("token-alice", "alice", "token-bob", "bob"));
