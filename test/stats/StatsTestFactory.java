...
/**
 * Factory for creating test instances of the Stats & Leaderboard slice (Slice D). Provides stubs
 * and in-memory implementations for all dependencies. Test time control is achieved by using
 * different {@link GameClock} durations:
 *
 * <ul>
 *   <li>Large duration → games are always active (for ongoing-game tests)
 *   <li>Duration = 1 ms → any game with an id smaller than the current time is completed
 * </ul>
 */
  ...
  private static class InMemoryPlayerGameRepository implements PlayerGameRepository {
    ...
    public List<PlayerGame> findPlayerGameByUsername(String username) {
      return new ArrayList<>(
          store.values().stream().filter(pg -> pg.username().equals(username)).toList());
    }
